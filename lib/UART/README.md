...
4. Запись первого байта данных:
   - Когда флаг готовности передатчика установлен, устанавливается флаг `USART_CR1_TCIE` (флаг прерывания по завершению передачи), чтобы быть уведомленным о завершении передачи данных.
...
5. Передача остальных данных:
   ...
   - Внутри цикла вызывается метод `Write` объекта `tx` с текущим значением `*data` (запись нового значения в кольцевой буфер передачи `tx`).
   ...
   - В случае, если метод `Write` вернул `false`, это говорит о том, что буфер полон и передача по какой-то причине не выполняется. Для борьбы с этим принудительно устанавливается флаг `USART_CR1_TCIE` (флаг прерывания по завершению передачи) для продолжения передачи.
...
1. Проверка номера UART:
   - Если `NUMBER_OF_UART` равен 1 (всегда), то выполняется код для инициализации GPIO для USART1.
...
4. Настройка режима работы пинов 9 и 10 порта GPIOA:
   - С помощью функции `MODIFY_REG` изменяются регистры `MODER` и AFR[1] порта GPIOA для настройки режима работы пинов 9 и 10.
   - Устанавливаются биты в полях `GPIO_MODER_MODE9` и `GPIO_MODER_MODE10` для установки пинов 9 и 10 в режим альтернативной функции.
   - Устанавливаются биты в полях `GPIO_AFRH_AFSEL9` и `GPIO_AFRH_AFSEL10` для выбора альтернативной функции USART1 на пинах 9 и 10.

5. Проверка состояния USART:
...
7. Настройка скорости передачи данных:
   - С помощью функции `MODIFY_REG` изменяется регистр `BRR` базового устройства для настройки делителя частоты передачи данных.
   - Значение делителя вычисляется как `*sys_clock / baud_rate`, где `*sys_clock` - системная тактовая частота, а `baud_rate` - желаемая скорость передачи данных.

8. Настройка количества бит данных:
...
10. Настройка количества стоп-битов:
...
12. Включение прерывания USART1:
    - Вызывается функция `NVIC_EnableIRQ` с аргументом `USART1_IRQn`, чтобы разрешить прерывание от USART1.
    - Устанавливается бит `USART_CR1_RXNEIE` для включения прерывания по приему
...
5. После обработки прерывания приема данных, проверяется условие `(READ_BIT(base->SR, USART_SR_TXE) == (USART_SR_TXE)) && (READ_BIT(base->CR1, USART_CR1_TCIE) == (USART_CR1_TCIE))`. Это условие проверяет, что флаг TXE (Transmit Data Register Empty) установлен и разрешено прерывание для передачи данных. Если это условие выполняется, то выполняется следующая инструкция:
   ...
   - Если функция `Read` возвращает `false`, то есть нет доступных данных для передачи, выполняется блок кода после оператора `else`:
      - В этом блоке кода сбрасывается флаг `TCIE` (Transmission Complete Interrupt Enable) в регистре управления `CR1` `USART`. Это отключает прерывание по завершению передачи.
